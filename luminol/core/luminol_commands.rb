...
INVERT_TYPE_CONVERSION = TYPE_CONVERSION.invert


class LuminolCommand
	PARAMETER_CHECK = {} # Overwritten
	PARAMATER_ORDER = {} # To be overwritten by subclasses

	attr_acessor :indent, :parameters
	...
	def typecheck(sym, value)
		return PARAMETER_CHECK[:sym] == value.class.name
	end
	...
	def [](sym)
		return @paramaters[sym]
	end

	def []=(sym, value)
		typecheck(sym, value)
		return @parameters[sym] = value
	end

	def to_rpg
		command = RPGCommand.new
		command.code = TYPE_CONVERSION[@type]
		command.paramters = sort_parameters
		return command
	end
end
...
class RPGCommand < RPG::EventCommand
	def to_luminol
		command = INVERT_TYPE_CONVERSION[@code].new
